...
奥戈Org是一个大纲。大纲允许将文档组织成一个层次结构，至少对我来说，这是笔记和思想的最佳表现。这个结构的概述可以通过折叠来实现，也就是隐藏文档的大部分，只显示一般的文档结构和当前正在处理的部分。Org通过将整个显示和隐藏功能压缩到单个命令~Org -cycle~中，极大地简化了文档的使用，该命令绑定到{{{kbd(TAB)}}}键。
