...
奥戈(Org)使用快速高效纯文本系统来记笔记、维护待办事项列表，做项目管理。她也是写作和出版系统，
她还支持源代码的“文字编程”和可重复研究。
...
本文档是[[info：org] [综合组织模式手册]]的压缩版本。 它包含所有基本功能和命令，以及重要的自定义提示。 
本手册适用于因篇幅庞大而无法阅读200页手册的初学者。
...
奥戈Org是一个大纲。大纲允许将文档组织成一个层次结构，至少对我来说，这是笔记和思想
的最佳表现。这个结构的概述可以通过折叠来实现，也就是隐藏文档的大部分，只显示一般的
文档结构和当前正在处理的部分。Org通过将整个显示和隐藏功能压缩到单个命令~Org -cycle~中，
极大地简化了文档的使用，该命令绑定到{{{kbd(TAB)}}}键。
...
标题定义了大纲的结构。奥戈的标题从左边空白处开始，有一个或多个星号，后面跟着一个空格。例如：
...
注意，以org-footnote-section命名的标题(默认为'Footnotes')被认为是特殊的。带有此标题的子树将被导出函数无声地忽略。
有些人觉得星号太多太吵了，他们更喜欢用空格后面跟着一个星号作为标题开头的轮廓。参见第15章杂项]，第38页的设置来实现这一点。
** 看见性循环
:PROPERTIES:
...
大纲可以折叠标题，以隐藏下面的文本。奥戈使用两个命令来改变可见性：{{{kbd(TAB)}}}和{{{kbd(S-TAB)}}}。
#+attr_texinfo: :sep ,
- {{{kbd(TAB)}}} ::
...
- {{{kbd(M-RET)}}} ::
  ...
  在当前位置插入新的同等等级的标题。如果当前是在一个列表项目，则会建立一个新的列表项。
  当在一行的中间使用这个命令，那么这行会分断，这行后面的部分会成为新的标题。
...
- {{{kbd(M-UP)}}}, {{{kbd(M-DOWN)}}} ::
  ...
  same level.
  在子标题里上下移动，例如：在当先的或者下一个子标题的相同等级转换。
...
文档尽可能折叠，但所选信息
与上方的标题结构一起可见[fn：3]。
只需尝试一下，您将立即看到它是如何工作的。

组织模式包含创建这些树的几个命令，所有这些
可以通过调度程序访问命令：
- {{{kbd(C-c /)}}} ::
...
在大纲树的一个项目里，手动格式列表能够提供额外的结构。他们也可以提供一个方法来创建
检查盒的列表。奥戈支持编辑这样的列表，而且每个导出器都能够识别他们。
...
相同的列表的项目必须具有相同的缩进。项在缩进的下一行（如项目符号/编号或更少）之前结束。
当所有项目都关闭时或在两个空白行之前，列表结束。
#+begin_example
,* Lord of the Rings
...
与HTML类似，奥戈提供了文件内部的链接，连接到其它文件的链接，论坛文章链接，
电子邮件等等很多。
奥戈会识别纯网络链接，可能是包裹在方括号内的，激活他们作为可以点击的链接。链接的一般格式为：
: [[链接][链接描述]]
或者：
: [[链接]]
** Internal links
...
奥戈提供了一些方法去正确地创建链接，在一个奥戈文件里插入链接，并且去跟踪链接。
...
buffer---see below.
主要的函数是 ~org-store-link~ ，叫做 {{{kbd(M-x org-store-link)}}} 。因为她很重要，
我们建议把它绑定为一个广泛使用的快捷键。 她存储了一个当前位置的链接。这个链接可以在另外一个
奥戈文件里插入。详见下面。
...
奥戈模式不需要TODO列表位于单独的文档中。
相反，TODO项是注释文件的一部分，因为TODO项通常在记录笔记时出现！
使用奥戈模式，只需将树中的任何条目标记为待办事项。 这样，信息就不会重复，
并且TODO项保留在它们出现的上下文中。
奥戈模式提供了一些方法，可让您概述从许多文件中收集的所有必需要做的事情。

** DONE 基本的计划功能 Basic TODO Functionality
:PROPERTIES:
...
给标题加上标签是很好的跨届信息，以实现标签和内容。奥戈模式对标签有很好的支持。
每个标题可以包含若干个标签，他们在标题的后面。
...
* Capture, Refile, Archive 捕获 归档 封存
:PROPERTIES:
...
compact and fast.
任何组织系统的一个主要部分是可以快速捕获灵感和任务，并且把相关资料综合在一起的能力。
奥戈模式使用叫做 “捕获” 的过程来完成它。捕获还能够存储任务的相关联的文件在一个特定
的目录里。一旦在系统里任务和项目需要移动。移动完整的项目文件到一个归档的文件，以保证系统的
整洁和快速。
** Capture
:PROPERTIES:
...
由于Org的工作方式，TODO项目，带时间戳的项目和带有标题的标题可能分散在整个文件甚至
多个文件中。 要获得未完成操作项目或对于特定日期重要的事件的概述，必须以组织方式收集
，分类和显示此信息。

提取的信息显示在特殊的/议程缓冲区/中。 该缓冲区是只读的，但提供命令来访问原始Org文件
中的相应位置，甚至可以远程编辑这些文件。 例如，从议程缓冲区进行远程编辑意味着您可以从
议程缓冲区中更改截止日期和约会的日期。 有关议程缓冲区中可用的命令，请参阅[[*议程缓冲区中的命令]]。
...
* Markup for Rich Contents 富文档的标记
:PROPERTIES:
...
奥戈主要是关于组织和搜索你的纯文本笔记。然而，她也提供了一个轻量级的支持富文本格式的标记语言。
通过联合使用额外的框架，你可以用奥戈写出漂亮的文档。
...
exported verbatim.
粗体：*cuti*, 斜体： /xieti斜体/, 下划线： _xhus分地方_, 逐字逐句： =fedfe废科举=
代码： ~dfe~ 。
